...
// NOTE: This class is copied from the source code. the difference: 103 line.

/** Runs lint via reflection in its own class loader */
class OKReflectiveLintRunner {
  ...
  companion object {
    ...
    private fun getLintClassLoader(gradle: Gradle, lintClassPath: Set<File>): ClassLoader {
      ...
      if (l == null) {
        ...
        // insert incrementlint to first
        val mergedUrls = mutableListOf<URL>()
        val insertFile = lintClassPath.first { it.name.startsWith("incrementlint") }
        mergedUrls.add(insertFile.toURI().toURL())
        mergedUrls.addAll(urls)

        l = DelegatingClassLoader(mergedUrls.toTypedArray())
        loader = l
      }
